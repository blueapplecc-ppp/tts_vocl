...
async def wait_for_event(
    ...
    while True:
        ...
        if msg.type == msg_type and msg.event == event_type:
            logger.info(f"Matched expected event: type={msg_type}, event={event_type}")
            return msg
        # 放宽限制：忽略未匹配的事件，继续读取，便于兼容服务端额外事件顺序
        logger.info(
            f"Waiting for {msg_type}/{event_type}, got {msg.type}/{msg.event}; ignoring and continuing."
        )
